...
<%
	...
	Page fenxiangPage = articleDao.getArticlePage(1, 12, "3");
	CachedRowSetImpl fenxiangRs = fenxiangPage.getCachedRowSetImpl();
	...
	int idInt = SYS.RequestIntByAttribute(request, "id", 0);//(request, "id", 0);
				...
				<div class='fl l_hot'>
					<div class="k_hot">
						...
						<ul class="top5box">
							<%
								int position = 1;
								while (fenxiangRs.next()) {
									String listPreview = LAB.replaceNull(fenxiangRs
											.getString("a_preview"));
									String listTitle = fenxiangRs.getString("a_title");
									String listAuthor = fenxiangRs.getString("a_author");
									String listTime = fenxiangRs.getString("a_time").substring(0, 10);
									int listId = fenxiangRs.getInt("a_id");
									String classStr = "";
									if (position <= 5)
										classStr = "hover";
							%>
