Fengs.add('kehuda/ui/calendar', function($){
	...
	Calendar.dayMouseDown = function(ev) {
		...
		cal.activeDiv = el;
		Calendar._C = cal;
		if (el.navtype != "year" && el.navtype != "month") with (Calendar) {
		...
		if (el.navtype == "month") {
			cal.timeout = setTimeout("Calendar.showMonthsCombo()", 10);
		} else if (el.navtype == "year") {
			cal.timeout = setTimeout((el.navtype > 0) ? "Calendar.showYearsCombo(true)" : "Calendar.showYearsCombo(false)", 10);
		} else {
			cal.timeout = null;
		}
		return Calendar.stopEvent(ev);
	...
	return;//注释双击效果;
		...
		if (el.id == "objclose" || el.id == "objmprev" || el.id == "objmnext" || el.id == "objyprev" || el.id == "objynext") el = el.parentNode; 
		if (el.navtype != "year" && el.navtype != "month") {
	...
	/*
	如果这个处理程序返回true,那么"约会"
	参数将被禁用。在这个例子中我们启用只有天范围内的10天从当前日期。
	可以使用函数date.getFullYear(),返回一年4位数的号码,date.getMonth(),返回月0 . . 11,获取当前日期日期(),返回日期的月为1 . .31日, 　　/ /使重计算在这里。然而,注意,这个函数　　/ /应该是非常快的,因为它是呼吁在一个月内,每一天　　/ /日历是(re)构造。
	*/
