...
<%
	...
	Page articlePage;
	if (type == 0) {
		articlePage = aManagerDao.getAllArticlePage(searchStr, pageNum, status,type);//(searchStr, pageNum);
	} else {
		articlePage = aManagerDao.getArticlePageByConditions(searchStr,pageNum, type, status);
	}
	...
	<div class='title'>
		...
		<div class='fr log_search'>
			...
			<div class='fr wrapSearch'>
				<input class='text' type="text" id="search" name="search" /> <input
					class='but' type="submit" value="" onclick="search()" />
			</div>
		</div>
	</div>
			...
			<tbody>
				...
				<%
					while (rs.next()) {
						...
						String typeDir = EditUtil.getTypeDirById(rs.getString("a_type"));
				%>
				<tr>
					<td><input type="checkbox" /></td>
					<td><img src="<%=rs.getString("a_preview")%>"></td>
					<td class='left'><a href="<%="../"+typeDir+"/"+ id+".html"%>"><%=a_title%></a></td>
					<td><%=typeStr%></td>
					<td><%=time%></td>
					<td><%=statusStr%></td>
					<td><%=clickNum%></td>
					<td><a class='revise'
						...
						+ rs.getString("a_id")%>"></a>
						<a class='delete'  onclick= "if(confirm( '是否删除！ ')==false)return   false; "  href="<%="../articleEditSubmit?do=delete&id=" + rs.getString("a_id")%>"></a></td>
				</tr>
				<%
					}
					rs.close();
