...
/*jslint browser: true */
/*jslint white: true */
Fengs.add('ui/nouislider', function(S, $){
	'use strict';
	S.use('ui/sfdsfs', function(D){
	...
	function closure ( target, options ){
		...
		function attach ( events, element, callback, data ) {
			...
			events.split(' ').forEach(function( eventName ){
				if (typeof element.addEventListener != "undefined") { 
					...
					element.attachEvent("on"+ eventName, method, false);
				}
		...
		function start ( event, data ) {
			...
			if ( data.handles.length === 1 ) {
				...
				if ( $(data.handles[0]).attr('disabled') ) {
					return false;
				}
			}
		...
		function tap ( event ) {
			...
			// Support 'disabled' handles
			if ( $(scope_Handles[handleNumber]).attr('disabled') ) {
				return false;
			}
