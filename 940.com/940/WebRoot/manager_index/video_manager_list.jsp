...
<%
	...
	Page articlePage;
	if (type == 0) {
		articlePage = videoDao.getAllVideoPage(searchStr, pageNum, status);//(searchStr, pageNum,status);
	} else {
	...
	System.out.println("current jsp article_manager_list type: " + type
			+ "");
...
<div class='r_content'>
	<div class='title'>
		...
		<div class='fr log_search'>
			<a class="fl" href="manager_index.jsp?go=video_edit&do=add"> 添加视频
			</a>
			<div class='fr wrapSearch'>
				<input class='text' type="text" id="search" /> <input class='but'
					type="submit" value=""  onclick="search()" />
			</div>
		</div>
	</div>
		...
		<table cellpadding="0" cellspacing="0">
			<thead>
				...
				out.print("selected");%>>隐藏</option>
					...
					<th>类型 <select name="type" id="type" onchange="search()">
							<option value="0" <%if (type < 0)
				out.print("selected");%>>全部</option>
							<option value="1" <%if (type == 1)
				out.print("selected");%>>会员</option>
							<option value="2" <%if (type == 2)
				out.print("selected");%>>免费</option>
							<option value="3" <%if (type == 3)
				out.print("selected");%>>热门</option>
							<option value="4" <%if (type == 4)
				out.print("selected");%>>最新</option>
					</select>
					</th>
					<th class='h8'>操作</th>
				</tr>
			...
			<tbody>
				<%
					while (rs.next()) {
						...
						String typeStr = EditUtil
								.getVideoTypeById(rs.getString("type"));
				...
				<tr>
					...
					<td><img style="width:100px;padding-right:0;"
						src="<%=preview%>"></td>
					<td class='left'><a href="<%="../video/"+id+".html" %>"><%=a_title%></a></td>
					<td><%=time%></td>
					<td><%=statusStr%></td>
					<td><%=typeStr%></td>
					<td><a class='revise'
						href="<%="manager_index.jsp?go=video_edit&do=edit&id="
						+ rs.getString("id")%>"></a><a
						class='delete'
						onclick="if(confirm( '是否删除！ ')==false)return   false; "
						href="<%="../videoEditSubmit?do=delete&id="
