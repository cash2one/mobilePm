Fengs.add('940/utils/slide', function(S, $ ){

	var slide = {
		init : function(){
			this.advert = $('#advert');
			this.wrap_slide = this.advert.find('.wrap_slide');
			this.slide = this.wrap_slide.children();
			this.iw = this.slide.width();
			this.objLen = this.slide.length;
			this.rou = $('#glass').find('.j_ro');
			this.i = 0;//索引
			this.off = true;
		...
		slideFn : function(){
			...
			$next.on('click',function(){
			 ...
			 this.ul.on({'mouseover':over,'mouseout':out});
			...
			function autosetIn(){
				...
				_this.ul.children().eq(_this.i==_this.objLen?0:_this.i).addClass('cur').siblings().removeClass('cur');
			...
			$last.on('click',function(){
				...
				_this.ul.children().eq(_this.i<0?(_this.objLen-1):_this.i).addClass('cur').siblings().removeClass('cur');
				( ie==true ? s.stand(_this.i) : s.ie(_this.i) );
				_this.off = false;
			})
			...
			var s = {
				...
				moveEnd : function(){
					if (ie) {//标准
						...
						_this.slide.eq(_this.i%_this.objLen).addClass('effect').siblings('div').removeClass('effect');
