 Fengs.add('940/video/index', function(S, $){
 	...
 	return {
 		...
 		getVideo : function(){
 				...
 				var videoList = {
 					videos : function(j,type){//热门视频
 						$.getJSON('/videoList?type='+type+'&page=1&line=10',function(d){
			 				...
			 				for (var i = 0; i < len; i++) {
			 					...
			 					aLi.eq(i).find('a')[0].vId = list[i]['id'];
			 					if (type==1) {
			 						aLi.eq(i).find('a')[0].vip = true;
			 					};
			 				};
 					...
 					vipFreeVideo : function(j,type,page,off){
 						 ...
 						 $.getJSON('/videoList',{type:type,page:page,line:12},function(d){
			 				...
			 				var list = d.list;
			 				if (!$.isArray(list)) {//如果只返回来一个数据需要做处理
			 					list = [$.parseJSON(list)];
			 				};
			 				console.log(list)
			 				var aHtml = '',aPage;
		 		...
		 		function play(){
		 			if (this.vip) {//vip视频要先登录才能看
 						if ($.cookie('940_COM_USER')==null||$.cookie('940_COM_PASS')==null) {
		 			...
		 			vTitle.html('正在播放：'+_title);
	 				 document.title = _title;
			 	 	 ...
			 	 	 window.location.href = '/video/'+this.vId+'.html';
