				...
				<table cellpadding="0" cellspacing="0">
					...
					%>
						<tr>
							...
							<td><a href="<%="../gonggao#id="+id%>"><%=titleStr %></a></td>
							<td><%=time %></td>
							<td><%=author %></td>
							<td><a class='revise' href="<%="manager_index.jsp?go=notice_edit&do=edit&id=" + id%>"></a><a class='delete'  onclick= "if(confirm( '是否删除！ ')==false)return   false; " href="<%="../noticeEditSubmit?do=delete&id=" + id%>"></a></td>
						</tr>
						<%}rs.close(); %>
					</tbody>
				</table>
