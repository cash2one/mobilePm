...
<%
	...
	Page sharePage;
	sharePage = shareDao.getSharePage(pageNum, 6, searchStr);//(pageNum, 6, searchStr,status);//(pageNum, 6);
					...
					<tbody>
						...
						<tr>
							...
							<td><a class='revise'  href="<%="manager_index.jsp?go=share_edit&do=edit&id="+ id%>"></a>
							 <a class='delete' onclick= "if(confirm( '是否删除！ ')==false)return   false; " href="<%="../shareSubmit?do=delete&id="+ id%>"></a></td>
						</tr>
						<%}rs.close(); %>
					</tbody>	
				</table>
